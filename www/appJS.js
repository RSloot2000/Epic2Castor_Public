...
// Centralized cell-color CSS classes — single source of truth
const CELL_CLASSES = "red-cell blue-cell orange-cell yellow-cell";
...
// ===== COLGROUP & SCROLLBAR HELPERS =====

/**
 * Ensure both header and body tables have a <colgroup> with the correct number of <col> elements.
 * Returns { headerColgroup: jQuery, bodyColgroup: jQuery, colCount: number } or null if tables not found.
 */
function ensureColgroups() {
    const $scrollWrapper = $('.dataTables_scroll').not('.modal .dataTables_scroll');
    ...
    if ($headerTable.length === 0 || $bodyTable.length === 0) {
        console.warn('[Colgroup] Header or body table not found');
        return null;
    }

    const colCount = $headerTable.find('thead th').length || $bodyTable.find('thead th').length || $headerTable.find('th').length;
    if (colCount === 0) return null;

    // Helper: create or update colgroup for a table
    function setupColgroup($table) {
        let $cg = $table.find('colgroup');
        if ($cg.length === 0) {
            $cg = $('<colgroup></colgroup>');
            $table.prepend($cg);
        }
        // Ensure correct number of <col> elements
        const existing = $cg.find('col').length;
        if (existing < colCount) {
            for (let i = existing; i < colCount; i++) {
                $cg.append('<col>');
            }
        } else if (existing > colCount) {
            $cg.find('col').slice(colCount).remove();
        }
        return $cg;
    }

    const $headerCG = setupColgroup($headerTable);
    const $bodyCG   = setupColgroup($bodyTable);

    return { headerColgroup: $headerCG, bodyColgroup: $bodyCG, colCount: colCount,
             headerTable: $headerTable, bodyTable: $bodyTable };
}

/**
 * Synchronise header–body column widths via <colgroup>.
 * Reads widths from header <col> elements and applies them to body <col> elements.
 */
function syncColgroupWidths() {
    const info = ensureColgroups();
    if (!info) return;

    const $headerCols = info.headerColgroup.find('col');
    const $bodyCols   = info.bodyColgroup.find('col');

    $headerCols.each(function(i) {
        const w = $(this).css('width') || $(this).attr('width');
        if (w) {
...
/**
 * Compensate header width for vertical scrollbar in body.
 * Instead of padding-right (which shifts sticky columns), we add a spacer <col>
 * or adjust the header table width to be narrower by the scrollbar width.
 */
function syncScrollbarCompensation() {
    const $scrollBody = $('.dataTables_scrollBody').not('.modal .dataTables_scrollBody');
    const $scrollHead = $('.dataTables_scrollHead').not('.modal .dataTables_scrollHead');

    if ($scrollBody.length === 0 || $scrollHead.length === 0) return;

    const bodyEl = $scrollBody[0];
    const scrollbarWidth = bodyEl.offsetWidth - bodyEl.clientWidth;

    // Reset any previous padding-based compensation
    $scrollHead.css({
        'padding-right': '0px',
        'box-sizing': 'border-box'
    });

    // Instead of adding padding to the header wrapper, we make the header table
    // narrower by the scrollbar width so it aligns perfectly with the body table.
    const $headerTable = $scrollHead.find('table');
    const $bodyTable   = $scrollBody.find('table');

    if ($headerTable.length > 0 && $bodyTable.length > 0) {
        const bodyWidth = $bodyTable[0].offsetWidth;
        if (scrollbarWidth > 0) {
            // Header table should be same width as body table (which is inside a narrower client area)
            $headerTable.css('width', bodyWidth + 'px');
            // Ensure the header wrapper itself matches the body wrapper (includes scrollbar space)
            $scrollHead.css('width', $scrollBody[0].offsetWidth + 'px');
            console.log('[ScrollbarSync] Header table narrowed to', bodyWidth + 'px, scrollbar:', scrollbarWidth + 'px');
        } else {
            // No scrollbar — header and body same width
            $scrollHead.css('width', $scrollBody.css('width'));
            $headerTable.css('width', $bodyTable.css('width'));
        }
    }

    // Re-sync colgroup widths after width adjustment
    syncColgroupWidths();
}
 ...
 * Uses <colgroup> elements for efficient width sync instead of per-row iteration.
...
function initColumnResizeSync() {
    ...
    const info = ensureColgroups();
    if (!info) {
        console.warn('[ColumnResize] Could not set up colgroups');
        return;
    }

    const { headerColgroup, bodyColgroup, colCount, headerTable, bodyTable } = info;
    const $headerCols = headerColgroup.find('col');
    const $bodyCols   = bodyColgroup.find('col');
    const stickyWidth = STICKY_COLUMNS_WIDTH;

    // Set initial colgroup widths from current header <th> widths
    headerTable.find('th').each(function(index) {
        const $th = $(this);
        const isSticky = index === 0 || index === colCount - 1;

        if (isSticky) {
            $headerCols.eq(index).css('width', stickyWidth);
            $bodyCols.eq(index).css('width', stickyWidth);
        } else {
            const w = $th.css('width') || ($th.outerWidth() + 'px');
            $headerCols.eq(index).css('width', w);
            $bodyCols.eq(index).css('width', w);
        }
    });

    console.log('[ColumnResize] Initialized colgroup-based resize sync for', colCount, 'columns');
}

/**
 * Initialize custom column resize grips
 * Creates draggable grips on column borders for precise resizing.
 * Uses <colgroup> for width changes — no per-row iteration needed.
 */
function initColumnResizeGrips() {
    ...
    $headerCols.each(function(index) {
        ...
        $grip.on('mousedown', function(e) {
            ...
            if (currentIndex === 0 || currentIndex === totalCols - 1 ||
                nextIndex === 0 || nextIndex === totalCols - 1) {
            ...
            // Read current widths from colgroup <col> elements for consistency
            const startWidthCurrent = parseFloat($hCols.eq(currentIndex).css('width')) || $th.outerWidth();
            const $nextTh = $headerCols.eq(nextIndex);
            const startWidthNext = parseFloat($hCols.eq(nextIndex).css('width')) || $nextTh.outerWidth();
            const minWidth = 50;
            
            // Mouse move: perform resize via colgroup (O(1) per frame)
            const onMouseMove = function(e) {
                ...
                // Apply to colgroup <col> elements — this resizes the entire column
                const wCurrent = newWidthCurrent + 'px';
                const wNext    = newWidthNext + 'px';
                
                $hCols.eq(currentIndex).css('width', wCurrent);
                $hCols.eq(nextIndex).css('width', wNext);
                $bCols.eq(currentIndex).css('width', wCurrent);
                $bCols.eq(nextIndex).css('width', wNext);
            };
            
            // Mouse up: end resize + re-sync scrollbar compensation
            const onMouseUp = function() {
                $grip.removeClass('resizing');
                $(document).off('mousemove', onMouseMove);
                $(document).off('mouseup', onMouseUp);
                
                // Re-sync scrollbar compensation after column widths changed
                syncScrollbarCompensation();
                
                console.log('[ResizeGrip] End resize - columns', currentIndex, '&', nextIndex);
            };
    ...
    console.log('[ResizeGrip] Initialized resize grips for', totalCols, 'columns');
}
...
function initTableResizeGrip() {
    ...
    $grip.on('mousedown', function(e) {
        ...
        const onMouseMove = function(moveEvent) {
            ...
            // Calculate column widths via colgroup
            ...
            if (colCount > 2) {
                const cgInfo = ensureColgroups();
                if (cgInfo) {
                    const availableWidth = newWidth - (parseInt(stickyWidth) * 2);
                    const middleColWidth = Math.floor(availableWidth / (colCount - 2));
                    const $hCols = cgInfo.headerColgroup.find('col');
                    const $bCols = cgInfo.bodyColgroup.find('col');
                    
                    for (let i = 0; i < colCount; i++) {
                        if (i === 0 || i === colCount - 1) {
                            $hCols.eq(i).css('width', stickyWidth);
                            $bCols.eq(i).css('width', stickyWidth);
                        } else {
                            $hCols.eq(i).css('width', middleColWidth + 'px');
                            $bCols.eq(i).css('width', middleColWidth + 'px');
                        }
                    }
                }
            }
        ...
        const onMouseUp = function(upEvent) {
            ...
            setTimeout(function() {
                ...
                // Re-sync scrollbar compensation after table resize
                syncScrollbarCompensation();
            }, 50);
        };
...
function convertLazyDropdownToSelect($div) {
    ...
    const tooltip = $div.attr('data-tooltip') || '';
    ...
    const tooltipAttr = tooltip ? ' data-tooltip="' + $('<div>').text(tooltip).html() + '"' : '';
...
function initializeSelect2OnElement($sel) {
    ...
    // dropdownParent = body so the dropdown is outside all scroll containers.
    // We position it with position:fixed afterwards, making it immune to
    // any ancestor-scroll that the browser's focus-management causes.
    $sel.select2({
        width: '100%',
        dropdownParent: $('body')
    });

    // ---- Scroll-lock + fixed-position dropdown ----
    // Problem: when Select2 focuses its search input the browser auto-scrolls
    // any ancestor (even body with overflow:hidden) to reveal the element.
    // This shifts the entire table layout.
    //
    // Fix (three layers):
    //   1. Capture-phase scroll listeners that instantly reset scroll
    //   2. Re-focus search input with { preventScroll: true }
    //   3. Position dropdown with position:fixed (independent of scroll)

    $sel.off('select2:opening').on('select2:opening', function() {
        // Snapshot every scroll position we care about
        var saved = {
            winY:      window.scrollY,
            bodyTop:   document.body.scrollTop,
            docElTop:  document.documentElement.scrollTop,
            tableBody: $('.dataTables_scrollBody').scrollTop()
        };
        $sel.data('_scrollLock', saved);

        // Capture-phase listener that fires before paint and resets scroll
        function onScroll() {
            window.scrollTo(0, saved.winY);
            document.body.scrollTop   = saved.bodyTop;
            document.documentElement.scrollTop = saved.docElTop;
        }
        window.addEventListener('scroll', onScroll, true);
        $sel.data('_scrollLockFn', onScroll);
    });

    $sel.off('select2:open').on('select2:open', function() {
        var saved    = $sel.data('_scrollLock') || {};
        var lockFn   = $sel.data('_scrollLockFn');

        // Remove the aggressive capture listener (its job is done)
        if (lockFn) window.removeEventListener('scroll', lockFn, true);

        // Re-focus search field WITHOUT triggering scroll
        ...
        // Belt-and-suspenders: force restore all scroll positions
        window.scrollTo(0, saved.winY || 0);
        document.body.scrollTop            = saved.bodyTop  || 0;
        document.documentElement.scrollTop = saved.docElTop || 0;
        var $sb = $('.dataTables_scrollBody');
        if ($sb.length && saved.tableBody !== undefined) {
            $sb.scrollTop(saved.tableBody);
        }

        // Position dropdown fixed — uses getBoundingClientRect so it is
        // correct regardless of any residual scroll offset.
        requestAnimationFrame(function() {
            // One more restore inside RAF (catches late scroll events)
            window.scrollTo(0, saved.winY || 0);
            document.body.scrollTop            = saved.bodyTop  || 0;
            document.documentElement.scrollTop = saved.docElTop || 0;
            if ($sb.length && saved.tableBody !== undefined) {
            ...
            var ddH  = $dd.outerHeight() || 300;

            // Decide direction: open below unless insufficient space
            var spaceBelow = vpH - rect.bottom;
            var spaceAbove = rect.top;
            var openBelow  = spaceBelow >= Math.min(ddH, 250) || spaceBelow >= spaceAbove;
            ...
            if (openBelow) {
                ...
                maxH = spaceAbove - 8;
                top  = Math.max(8, rect.top - Math.min(ddH, maxH));
            }
            ...
            $dd.find('.select2-results').css({
                maxHeight: (maxH - 40) + 'px',
                overflowY: 'auto'
            });
        });
    });
    ...
    $sel.off('select2:close').on('select2:close', function() {
        // Remove scroll-lock listener if it somehow survived
        var lockFn = $sel.data('_scrollLockFn');
        if (lockFn) window.removeEventListener('scroll', lockFn, true);
        $sel.removeData('_scrollLock').removeData('_scrollLockFn');

        // Final safety: body must never be scrolled in table mode
        window.scrollTo(0, 0);
        document.body.scrollTop = 0;
        document.documentElement.scrollTop = 0;

        // Re-apply CSS class after close (Select2 can reset styles)
        var cls = $(this).data('cssclass');
        if (cls && cls.trim() !== "") {
            setTimeout(function() {
                $sel.next('.select2-container')
                    .find('.select2-selection')
                    .removeClass(CELL_CLASSES)
                    .addClass(cls);
            }, 10);
        }
    });
    
    // Apply CSS class and tooltip after select2 initialization
    const cssClass = $sel.data('cssclass');
    const selTooltip = $sel.attr('data-tooltip') || '';
    if ((cssClass && cssClass.trim() !== "") || selTooltip) {
        ...
        setTimeout(function() {
            ...
            if (cssClass && cssClass.trim() !== "") {
                $selection.removeClass(CELL_CLASSES).addClass(cssClass);
                $sel.removeClass(CELL_CLASSES).addClass(cssClass);
            }
            // Propagate tooltip to Select2 container so CSS :hover works
            if (selTooltip) {
                $container.attr('data-tooltip', selTooltip);
            }
        }, 10);
    }
 ...
 * Debounced via requestAnimationFrame to coalesce rapid calls
 * (drawCallback + reloadSelect2 can fire within the same frame)
 */
function initializeSelect2Elements() {
    // Debounce: if already scheduled for this frame, skip
    if (window._select2InitPending) return;
    window._select2InitPending = true;

    requestAnimationFrame(function() {
        window._select2InitPending = false;
...
$(document).ready(function() {
    ...
    $(document).on('draw.dt', '#table table', function() {
        ...
        // Safety net: reset any accidental window scroll that may have been
        // caused by DOM changes (Select2 destroy/recreate, focus shifts, etc.)
        // With overflow:hidden on body the window should never be scrolled.
        if (window.scrollY !== 0) {
            window.scrollTo(0, 0);
        }
...
// Only save when body is actually scrollable (not overflow:hidden) to prevent
// stale values from programmatic scrolls that shift the table under the header
window.addEventListener("scroll", function() {
    const bodyOverflow = window.getComputedStyle(document.body).overflow;
    if (bodyOverflow !== 'hidden' && window.scrollY >= 0) {
...
window.resizeTableToWindow = function() {
    ...
    // Get both header and body tables (only from the main page, NOT from modals)
    const $scrollWrapper = $(".dataTables_scroll").not(".modal .dataTables_scroll");
    const $headerTable = $scrollWrapper.find(".dataTables_scrollHead table");
    const $bodyTable = $scrollWrapper.find(".dataTables_scrollBody table");
    ...
    // Get scroll wrappers first (only main page, not modal)
    let $scrollHead = $('.dataTables_scrollHead').not('.modal .dataTables_scrollHead');
    let $scrollBody = $('.dataTables_scrollBody').not('.modal .dataTables_scrollBody');
    ...
    // Calculate and set column widths for even distribution via <colgroup>
    const stickyWidth = STICKY_COLUMNS_WIDTH;
    const colCount = $headerTable.find("th").length || $bodyTable.find("th").length;
    
    // Ensure colgroups exist for both tables
    const cgInfo = ensureColgroups();
    
    if (colCount > 2 && cgInfo) {
        ...
        // Apply widths to colgroup <col> elements — this controls both header and body
        for (let i = 0; i < colCount; i++) {
            if (i === 0 || i === colCount - 1) {
                // Sticky columns
                $hCols.eq(i).css('width', stickyWidth);
...
try {
    Shiny.addCustomMessageHandler("resizeDiv", function(message) {
        ...
        $("#scrollDiv").css("overflow", "visible");
        
        // Sync scrollbar compensation after resize
        requestAnimationFrame(function() {
            syncScrollbarCompensation();
        });
    });
...
// Restore scroll position from localStorage (one-shot after full render / page reload)
try {
    Shiny.addCustomMessageHandler("restoreScroll", function(message) {
        const pos = localStorage.getItem("scrollPos");
        // One-shot: remove the stored value so it is not re-applied on subsequent calls
        localStorage.removeItem("scrollPos");

        // Only restore when body can actually scroll (not overflow:hidden)
        const bodyOverflow = window.getComputedStyle(document.body).overflow;
        if (bodyOverflow === 'hidden') {
            // Body is not scrollable — force scroll to 0 as safety net
            window.scrollTo(0, 0);
            return;
        }

        if (pos !== null && parseInt(pos) > 0) {
            setTimeout(function() {
...
Shiny.addCustomMessageHandler("eval", function(code) {
    try { eval(code); } catch(e) { console.warn("eval handler:", e); }
});
    ...
    if ($element.length > 0 && $element.is('select')) {
        ...
        if ($element.hasClass('select2-hidden-accessible')) {
            ...
            if ($container.length > 0) {
                const $selection = $container.find('.select2-selection');
                $selection.removeClass(CELL_CLASSES);
                if (cssClass) {
        ...
        // Update CSS class
        $element.removeClass(CELL_CLASSES);
        if (cssClass) {
...
// Lightweight cell-color + tooltip update (no value change, no re-render)
Shiny.addCustomMessageHandler("updateCellColor", function(message) {
    const row = message.row;
    const col = message.col;
    const cssClass = message.cssClass || '';
    ...
    if ($element.is('select')) {
        ...
        $element.removeClass(CELL_CLASSES).addClass(cssClass);
        const $container = $element.next('.select2-container');
        if ($container.length) {
            $container.find('.select2-selection').removeClass(CELL_CLASSES).addClass(cssClass);
            // Tooltip on Select2 container
            if (tooltip) {
                $container.attr('data-tooltip', tooltip);
            } else {
                $container.removeAttr('data-tooltip');
            }
        }
    } else if ($element.hasClass('lazy-dropdown')) {
        // Still a div placeholder
        $element.removeClass(CELL_CLASSES).addClass(cssClass);
        $element.attr('data-cssclass', cssClass);
    }
    
    // Update tooltip on the source element
    if (tooltip) {
...
// ===== CELL TOOLTIP (JS-based, appended to <body>) =====
// Avoids overflow:hidden clipping inside <td> elements.
(function() {
    let $tip = null;
    ...
    function showTip(e) {
        ...
        if (!$tip) {
            $tip = $('<div class="cell-tooltip"></div>').appendTo('body');
        }
        
        // Determine color variant class
        let tipColor = '';
        if (el.classList.contains('red-cell'))    tipColor = 'tip-red';
        else if (el.classList.contains('blue-cell'))   tipColor = 'tip-blue';
        else if (el.classList.contains('orange-cell')) tipColor = 'tip-orange';
        else if (el.classList.contains('yellow-cell')) tipColor = 'tip-yellow';
        // Also check Select2 selection inside container
        if (!tipColor) {
            const sel = el.querySelector && el.querySelector('.select2-selection');
            if (sel) {
                if (sel.classList.contains('red-cell'))    tipColor = 'tip-red';
                else if (sel.classList.contains('blue-cell'))   tipColor = 'tip-blue';
                else if (sel.classList.contains('orange-cell')) tipColor = 'tip-orange';
                else if (sel.classList.contains('yellow-cell')) tipColor = 'tip-yellow';
            }
        }
        
        $tip.text(text)
            .removeClass('tip-red tip-blue tip-orange tip-yellow')
            .addClass(tipColor)
            .show();
        ...
        let top  = rect.top - $tip.outerHeight() - 6;
    ...
    // Delegated events on #table for both lazy-dropdowns and select2 containers
    $(document).on('mouseenter', '[data-tooltip]', showTip);
    $(document).on('mouseleave', '[data-tooltip]', hideTip);
})();
