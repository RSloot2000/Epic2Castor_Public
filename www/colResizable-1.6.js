/**
	...
	THIS PLUGIN HAS BEEN MODIFIED FOR THE EPIC2CASTOR PROJECT	
*/
...
(function($) {
    ...
    // Als er al een touchstart special bestaat, houd de originele setup bij
    var origSetup = ($.event.special.touchstart && $.event.special.touchstart.setup) || null;

    $.event.special.touchstart = {
        setup: function(data, namespaces, eventHandle) {
            // Gebruik native addEventListener met passive-opties
            this.addEventListener("touchstart", eventHandle, supportsPassive ? { passive: true } : false);
            // Roep eventueel de originele setup aan (indien van toepassing)
            if (origSetup) { return origSetup.apply(this, arguments); }
        },
        teardown: function(namespaces) {
            this.removeEventListener("touchstart", $.event.special.touchstart.handler);
        }
    };
})(jQuery);
...
(function($){ 	
	...
	var init = function( tb, options){	
        ...
        t.addClass(SIGNATURE).attr(ID, id); 
        var gripsContainer = $('<div class="JCLRgrips"/>'); 
        if(t.find('thead').length > 0){ 
          t.find('thead').append(gripsContainer); 
          t.gc = gripsContainer;  // gebruik dit als de container in de thead zit
        } else { 
          t.before(gripsContainer); 
          t.gc = t.prev(); 
        }
        t.g = []; t.c = []; t.w = t.width(); t.f=t.opt.fixed;	//t.c and t.g are arrays of columns and grips respectively				
	...
	var createGrips = function(t){	
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			if (!dc) {
				...
				g.on('mousedown', onGripMouseDown);
				g.get(0).addEventListener('touchstart', onGripMouseDown, { passive: true });
			} else {
	...
	*/
	...
	var onGripMouseDown = function(e){
		...
		d.on('touchmove.'+SIGNATURE+' mousemove.'+SIGNATURE, onGripDrag)
		 .on('touchend.'+SIGNATURE+' mouseup.'+SIGNATURE, onGripDragOver);
