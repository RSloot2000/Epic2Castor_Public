...
library(shinyFiles)      # Server-side file browsing (SharePoint/OneDrive)
...
# Load UI helper functions (keyboard shortcuts section generator)
source(epc_path("ui_helpers_script"))
...
# "manual_values" is an internal table for manual value management
hidden_tables <- c("variabelen", "elements_backup_before_test", "manual_values")
                      ...
                      div(class = "menu-bar",
                          ...
                          div(class = "dropdown menu-group",
                              ...
                              tags$ul(class = "dropdown-menu",
                                      ...
                                      tags$li(uiOutput("sp_reload_menu_item", inline = TRUE))
                              )
                          ),
...
server <- function(input, output, session) {
  ...
  # ============================================================================
  # CELL STATUS DETERMINATION  (colour + tooltip in one place)
  # ============================================================================
  #' Determine CSS class, status label and tooltip for a single cell.
  #'
  #' Priority: duplicate (orange) > warning/yellow > manual (blue) > invalid (red) > valid ("")
  #'
  #' @param value           Current cell value (character scalar)
  #' @param options_list    Valid options for this column (character vector)
  #' @param column          Column name
  #' @param row_idx         Row number (1-based)
  #' @param file            Active table name (e.g. "elements")
  #' @param duplicate_info  Result of detect_duplicate_elements()
  #' @param manual_vals     Named list (key = "row_col", value = TRUE)
  #' @param full_row        Named list/vector of the current row (for cross-column checks)
  #' @return list(css_class, status, tooltip)
  determine_cell_status <- function(value, options_list, column, row_idx, file,
                                    duplicate_info, manual_vals, full_row = NULL) {
    # --- 1. Duplicate check (highest priority) ---
    if (file == "elements") {
      if (column == "Element" && row_idx %in% duplicate_info$duplicate_rows_element) {
        return(list(css_class = "orange-cell", status = "duplicate",
                    tooltip = paste0("Duplicate Element: ", value)))
      }
      if (column == "castor_kolom" && row_idx %in% duplicate_info$duplicate_rows_castor) {
        return(list(css_class = "orange-cell", status = "duplicate",
                    tooltip = paste0("Duplicate Castor name: ", value)))
      }
    }

    # --- 2. Empty cell \u2014 check for incomplete mapping (warning) ---
    if (!nzchar(value)) {
      if (file == "elements" && !is.null(full_row)) {
        other <- if (column == "Element") full_row[["castor_kolom"]] else if (column == "castor_kolom") full_row[["Element"]] else NULL
        if (!is.null(other) && nzchar(other)) {
          return(list(css_class = "yellow-cell", status = "incomplete",
                      tooltip = paste0("Incomplete mapping: ", column, " is empty")))
        }
      }
      return(list(css_class = "", status = "empty", tooltip = ""))
    }

    # --- 3. Value in valid options \u2192 valid ---
    if (value %in% options_list) {
      return(list(css_class = "", status = "valid", tooltip = ""))
    }

    # --- 4. Fuzzy match check (warning) ---
    # Skip fuzzy matching for very short values (<=2 chars):
    # agrep max.distance=0.2 translates to ceil(0.2*nchar) allowed edits.
    # For 1-char values that's ceil(0.2)=1 edit, matching ANY single char
    # against ANY other (e.g. "6" matches "0"). Too aggressive.
    if (nchar(value) > 2) {
      fuzzy_hits <- agrep(value, options_list, max.distance = 0.2, value = TRUE, ignore.case = TRUE)
      if (length(fuzzy_hits) > 0) {
        suggestion <- fuzzy_hits[1]
        return(list(css_class = "yellow-cell", status = "fuzzy",
                    tooltip = paste0("Near match: did you mean \u2018", suggestion, "\u2019?")))
      }
    }

    # --- 5. Manual value (blue) ---
    key <- paste0(row_idx, "_", column)
    if (!is.null(manual_vals[[key]]) && isTRUE(manual_vals[[key]])) {
      return(list(css_class = "blue-cell", status = "manual",
                  tooltip = "Manually entered value"))
    }

    # --- 6. Invalid (red) ---
    return(list(css_class = "red-cell", status = "invalid",
                tooltip = paste0("Value \u2018", value, "\u2019 not found in options list")))
  }
      ...
      later::later(function() {
        ...
        if (!is.null(info$script_type)) {
            if (info$script_type == "baseline") {
              ...
              env_vars <- c(EPIC2CASTOR_LOGDIR = getOption("epic2castor.logdir", Sys.getenv("EPIC2CASTOR_LOGDIR", "")))
              temp_input <- Sys.getenv("EPC_TEMP_INPUT_DIR", "")
              if (nzchar(temp_input)) env_vars <- c(env_vars, EPC_TEMP_INPUT_DIR = temp_input)
              proc <- process$new(
                "Rscript",
                args = c(scriptPath),
                stdout = "|",
              ...
              output$baseline_progressbar <- renderUI({
                div(class = "progress", style = "height: 20px; background:#eee;",
                    div(id = "baseline_pbar", class = "progress-bar progress-bar-striped active", role = "progressbar",
                        style = "width: 0%;",
              ...
              .bl2_poll <- new.env(parent = emptyenv()); .bl2_poll$pct <- -1L; .bl2_poll$step <- ""
              ...
              observerHandle <- observe({
                ...
                tryCatch({
                  ...
                  if (file.exists(status_path)) {
                    ...
                    if (!inherits(st, "try-error") && is.list(st)) {
                      ...
                      if (!runnerState$error_detected) {
                        rpct <- round(pct)
                        if (rpct != .bl2_poll$pct || step != .bl2_poll$step) {
                          .bl2_poll$pct <- rpct; .bl2_poll$step <- step
                          .update_pbar("baseline_pbar", pct)
                        }
                      }
                      ...
                      if (!runnerState$error_detected) .update_status("baseline_status_line", line)
                    }
                  }
                  ...
                  .update_status("baseline_status_line", paste("Monitor error:", conditionMessage(e)))
              ...
              output$biobank_progressbar <- renderUI({
                div(class = "progress", style = "height: 20px; background:#eee;",
                    div(id = "biobank_pbar", class = "progress-bar progress-bar-striped active", role = "progressbar",
                        style = "width: 0%;",
                        `aria-valuemin` = 0, `aria-valuemax` = 100,
                        "0%")
                )
              ...
              .bb2_poll <- new.env(parent = emptyenv()); .bb2_poll$pct <- -1L; .bb2_poll$step <- ""
                ...
                tryCatch({
                  ...
                  if (file.exists(status_path)) {
                    ...
                    if (!inherits(st, "try-error") && is.list(st)) {
                      ...
                      if (!runnerState$error_detected) {
                        rpct <- round(pct)
                        if (rpct != .bb2_poll$pct || step != .bb2_poll$step) {
                          .bb2_poll$pct <- rpct; .bb2_poll$step <- step
                          .update_pbar("biobank_pbar", pct)
                        }
                      }
                      line <- paste(step, if (nzchar(detail)) paste("-", detail) else "", if (!is.null(eta_txt)) paste("(", eta_txt, ")") else "")
                      if (!runnerState$error_detected) .update_status("biobank_status_line", line)
                    }
                  }
                  ...
                  .update_status("biobank_status_line", paste("Monitor error:", conditionMessage(e)))
                  updateRunnerFooter(TRUE)
                  observerHandle$destroy()
                  if (!isTRUE(runnerState$canceled) && !is.null(info$on_complete)) later::later(info$on_complete, delay = 0.05)
                })
  ...
  # --- Ensure manual_values table exists and load persisted values ---
  dbExecute(con, "CREATE TABLE IF NOT EXISTS manual_values (
    file TEXT NOT NULL,
    cell_key TEXT NOT NULL,
    value TEXT,
    PRIMARY KEY (file, cell_key)
  )")

  #' Load manual values from SQLite for the given file/table
  load_manual_values <- function(file_name) {
    rows <- dbGetQuery(con, "SELECT cell_key, value FROM manual_values WHERE file = ?",
                       params = list(file_name))
    vals <- list()
    if (nrow(rows) > 0) {
      for (r in seq_len(nrow(rows))) {
        vals[[ rows$cell_key[r] ]] <- TRUE
      }
    }
    vals
  }

  #' Save a single manual value to SQLite
  save_manual_value <- function(file_name, cell_key, value) {
    dbExecute(con, "INSERT OR REPLACE INTO manual_values (file, cell_key, value) VALUES (?, ?, ?)",
              params = list(file_name, cell_key, as.character(value)))
  }
  ...
  # Performance helpers: update progress bar and status text via JS (bypasses reactive graph)
  .update_pbar <- function(id, pct, done = FALSE) {
    if (isTRUE(done)) {
      session$sendCustomMessage("eval", sprintf("$('#%s').removeClass('progress-bar-striped active').addClass('progress-bar-success').css({'width':'100%%','background-color':'#5cb85c'}).text('100%%')", id))
    } else {
      rpct <- round(pct)
      session$sendCustomMessage("eval", sprintf("$('#%s').css('width','%s%%').text('%s%%')", id, rpct, rpct))
    }
  }
  .update_status <- function(id, text) {
    session$sendCustomMessage("eval", sprintf("document.getElementById('%s').textContent=%s", id, jsonlite::toJSON(as.character(text), auto_unbox = TRUE)))
  }
  ...
  session$onSessionEnded(function() {
    ...
    # Clean up SharePoint temp files and password
    temp_sp <- file.path(tempdir(), "epc_sharepoint")
    if (dir.exists(temp_sp)) try(unlink(temp_sp, recursive = TRUE, force = TRUE), silent = TRUE)
    Sys.unsetenv("EPC_TEMP_INPUT_DIR")
    try({ sp_session$password <- NULL }, silent = TRUE)
  ...
  observeEvent(input$file, {
    ...
    # Load persisted manual values for the new table
    manualValues$vals <- load_manual_values(input$file)
  ...
  runnerState$forced_completion <- FALSE
    ...
    observerHandle <- observe({
      ...
      if (is.null(status)) {
        if (!runnerState$error_detected) .update_status("castor_status_line", "Retrieving metadata\u2026")
        return()
      }
  ...
  run_baseline_script <- function(on_complete = NULL) {
    ...
    scriptPath <- file.path(epc_path("baseline_scripts_dir"), "baseline.r")
    env_vars <- c(EPIC2CASTOR_LOGDIR = getOption("epic2castor.logdir", Sys.getenv("EPIC2CASTOR_LOGDIR", "")))
    # Pass temp input dir to subprocess when data was imported from SharePoint
    temp_input <- Sys.getenv("EPC_TEMP_INPUT_DIR", "")
    if (nzchar(temp_input)) env_vars <- c(env_vars, EPC_TEMP_INPUT_DIR = temp_input)
    proc <- process$new(
      "Rscript",
    ...
    output$baseline_status_line <- renderText({ "Starting…" })
    .bl_poll <- new.env(parent = emptyenv()); .bl_poll$pct <- -1L; .bl_poll$step <- ""
          ...
          if (!inherits(st, "try-error") && is.list(st)) {
            ...
            if (!runnerState$error_detected) {
              ...
              if (!is.na(step_lc) && identical(step_lc, "done")) {
                if (.bl_poll$step != "done") { .bl_poll$step <- "done"; .update_pbar("baseline_pbar", 100, done = TRUE) }
              } else if (rpct != .bl_poll$pct || step != .bl_poll$step) {
                .bl_poll$pct <- rpct; .bl_poll$step <- step
                .update_pbar("baseline_pbar", pct)
              }
            }
            line <- paste(step, if (nzchar(detail)) paste("-", detail) else "", if (!is.null(eta_txt)) paste("(", eta_txt, ")") else "")
  ...
  run_follow_up_script <- function(on_complete = NULL) {
    ...
    output$follow_up_progressbar <- renderUI({
      div(class = "progress", style = "height: 20px; background:#eee;",
          div(id = "follow_up_pbar", class = "progress-bar progress-bar-striped active", role = "progressbar",
              style = "width: 0%;",
              `aria-valuemin` = 0, `aria-valuemax` = 100,
              "0%")
      )
    ...
    .fu_poll <- new.env(parent = emptyenv()); .fu_poll$pct <- -1L; .fu_poll$step <- ""
    ...
    observerHandle <- observe({
      ...
      tryCatch({
        ...
        if (file.exists(status_path)) {
          ...
          if (!inherits(st, "try-error") && is.list(st)) {
            ...
            if (!runnerState$error_detected) {
              rpct <- round(pct)
              if (!is.na(step_lc) && identical(step_lc, "done")) {
                if (.fu_poll$step != "done") { .fu_poll$step <- "done"; .update_pbar("follow_up_pbar", 100, done = TRUE) }
              } else if (rpct != .fu_poll$pct || step != .fu_poll$step) {
                .fu_poll$pct <- rpct; .fu_poll$step <- step
                .update_pbar("follow_up_pbar", pct)
              }
            }
            line <- paste(step, if (nzchar(detail)) paste("-", detail) else "", if (!is.null(eta_txt)) paste("(", eta_txt, ")") else "")
            if (!runnerState$error_detected) .update_status("follow_up_status_line", line)
          }
        }
        ...
        .update_status("follow_up_status_line", paste("Monitor error:", conditionMessage(e)))
        updateRunnerFooter(TRUE)
        observerHandle$destroy()
        runnerState$proc <- NULL
        runnerState$observer <- NULL
  ...
  run_biobank_script <- function(on_complete = NULL) {
    ...
    output$biobank_status_line <- renderText({ "Starting…" })
    .bb_poll <- new.env(parent = emptyenv()); .bb_poll$pct <- -1L; .bb_poll$step <- ""
        ...
        if (file.exists(status_path)) {
          ...
          if (!inherits(st, "try-error") && is.list(st)) {
            ...
            if (!runnerState$error_detected) {
              rpct <- round(pct)
              if (!is.na(step_lc) && identical(step_lc, "done")) {
                if (.bb_poll$step != "done") { .bb_poll$step <- "done"; .update_pbar("biobank_pbar", 100, done = TRUE) }
              } else if (rpct != .bb_poll$pct || step != .bb_poll$step) {
                .bb_poll$pct <- rpct; .bb_poll$step <- step
                .update_pbar("biobank_pbar", pct)
              }
            }
            line <- paste(step, if (nzchar(detail)) paste("-", detail) else "", if (!is.null(eta_txt)) paste("(", eta_txt, ")") else "")
  ...
  # ============================================================================
  # LAB DATA SCRIPT RUNNER
  # ============================================================================
  # Executes the lab_data.r script with real-time progress monitoring.
  # Similar to biobank runner, monitors status.json for progress updates.
  #
  # @param on_complete Optional callback function to execute after successful completion
  
  run_lab_data_script <- function(on_complete = NULL) {
    removeModalSafe()
    updateRunnerFooter(FALSE)
    {
      run_dir <- getOption("epic2castor.logdir", Sys.getenv("EPIC2CASTOR_LOGDIR", ""))
    ...
    runnerState$canceled <- FALSE
    showModalSafe(modalDialog(
      title = "Running Lab Data",
      tagList(
        uiOutput("lab_data_progressbar"),
        div(style = "margin-top:8px;", textOutput("lab_data_status_line"))
      ),
      footer = uiOutput("runner_footer"),
      easyClose = FALSE,
      size = "l"
    ...
    scriptPath <- file.path(epc_path("scripts_dir"), "lab_data", "lab_data.r")
    env_vars <- c(EPIC2CASTOR_LOGDIR = getOption("epic2castor.logdir", Sys.getenv("EPIC2CASTOR_LOGDIR", "")))
    temp_input <- Sys.getenv("EPC_TEMP_INPUT_DIR", "")
    if (nzchar(temp_input)) env_vars <- c(env_vars, EPC_TEMP_INPUT_DIR = temp_input)
    proc <- process$new(
    ...
    output$lab_data_progressbar <- renderUI({
      div(class = "progress", style = "height: 20px; background:#eee;",
          div(id = "lab_data_pbar", class = "progress-bar progress-bar-striped active", role = "progressbar",
              style = "width: 0%;",
              `aria-valuemin` = 0, `aria-valuemax` = 100,
              "0%")
      )
    })
    output$lab_data_status_line <- renderText({ "Starting\u2026" })
    .ld_poll <- new.env(parent = emptyenv()); .ld_poll$pct <- -1L; .ld_poll$step <- ""
    ...
    observerHandle <- observe({
      ...
      tryCatch({
        ...
        if (length(out_lines)) lapply(out_lines, function(x) cat(paste0("[LabData][OUT] ", x, "\n")))
        if (length(err_lines)) lapply(err_lines, function(x) cat(paste0("[LabData][ERR] ", x, "\n")))
        combined_lines <- c(out_lines, err_lines)
        if (length(combined_lines)) {
          err_line <- detect_error_line(combined_lines)
          if (!is.null(err_line)) {
            update_runner_error(err_line, "lab_data_status_line", "lab_data_progressbar")
          }
        }
        ...
        if (file.exists(status_path)) {
          ...
          if (!inherits(st, "try-error") && is.list(st)) {
            ...
            if (!runnerState$error_detected) {
              rpct <- round(pct)
              if (!is.na(step_lc) && identical(step_lc, "done")) {
                if (.ld_poll$step != "done") { .ld_poll$step <- "done"; .update_pbar("lab_data_pbar", 100, done = TRUE) }
              } else if (rpct != .ld_poll$pct || step != .ld_poll$step) {
                .ld_poll$pct <- rpct; .ld_poll$step <- step
                .update_pbar("lab_data_pbar", pct)
              }
            }
            line <- paste(step, if (nzchar(detail)) paste("-", detail) else "", if (!is.null(eta_txt)) paste("(", eta_txt, ")") else "")
            if (!runnerState$error_detected) .update_status("lab_data_status_line", line)
          }
        }
        ...
        if (!proc$is_alive()) {
          ...
          try({
            leftover_out <- proc$read_output_lines()
            leftover_err <- proc$read_error_lines()
            if (length(leftover_out)) lapply(leftover_out, function(x) cat(paste0("[LabData][OUT] ", x, "\n")))
            if (length(leftover_err)) lapply(leftover_err, function(x) cat(paste0("[LabData][ERR] ", x, "\n")))
            leftover_lines <- c(leftover_out, leftover_err)
          ...
          if (!script_completed_successfully && (is.na(exit_status) || exit_status != 0) && !runnerState$error_detected && !is_windows_crash) {
            msg <- sprintf("Lab Data script failed (exit %s). Check logs for details.", as.character(exit_status))
            update_runner_error(msg, "lab_data_status_line", "lab_data_progressbar")
          }
          
          if (!script_completed_successfully && !is.na(exit_status) && exit_status != 0 && !is_windows_crash) {
            tryCatch({
              safeNotify(sprintf("Lab Data script exited with status %s", as.character(exit_status)), "error")
            }, error = function(e) {
              cat(sprintf("[LabData] Could not show notification: %s\n", conditionMessage(e)))
            })
          }
        ...
        .update_status("lab_data_status_line", paste("Monitor error:", conditionMessage(e)))
        updateRunnerFooter(TRUE)
        observerHandle$destroy()
        runnerState$proc <- NULL
        runnerState$observer <- NULL
  ...
  # ============================================================================
  # Allows user to select which output scripts to run (baseline, follow-up, biobank, lab data).
  ...
  observeEvent(input$run_main_script, {
    showModalSafe(modalDialog(
      ...
      tagList(
        ...
        checkboxInput("select_lab_data", "Lab Data", value = FALSE)
      ),
      footer = tagList(
        modalButton("Cancel"),
  ...
  observeEvent(input$confirm_run_selected_scripts, {
    ...
    if (isTRUE(input$select_lab_data))  tasks <- c(tasks, "lab_data")
    ...
    run_chain <- function(i) {
      ...
      if (t == "baseline") {
        ...
        run_lab_data_script(on_complete = function() run_chain(i + 1))
      } else {
      ...
      tagList(
        fluidRow(
          column(12,
                 ...
                 checkboxInput("upload_select_lab_data", "Lab Data", value = FALSE),
  ...
  observeEvent(input$confirm_upload, {
    ...
    if (isTRUE(input$upload_select_lab_data))  tasks <- c(tasks, "lab_data")
    ...
    run_one_upload <- function(task, on_done) {
      implemented <- task %in% c("baseline", "biobank", "followup", "lab_data")
      if (!implemented) {
  ...
  updateRunnerFooter(FALSE)
      ...
      scriptPath <- switch(task,
           ...
           lab_data  = file.path(epc_path("scripts_dir"), "lab_data", "lab_dataExport.r"),
      ...
      output$upload_progressbar <- renderUI({
        div(class = "progress", style = "height: 20px; background:#eee;",
            div(id = "upload_pbar", class = "progress-bar", role = "progressbar",
                style = "width: 0%;",
                `aria-valuemin` = 0, `aria-valuemax` = 100,
                "0%")
        )
      ...
      .up_poll <- new.env(parent = emptyenv()); .up_poll$pct <- -1L; .up_poll$step <- ""
        ...
        tryCatch({
          if (file.exists(status_path)) {
            ...
            if (!inherits(st, "try-error") && is.list(st)) {
              ...
              rpct <- round(pct)
              if (rpct != .up_poll$pct || step != .up_poll$step) {
                .up_poll$pct <- rpct; .up_poll$step <- step
                .update_pbar("upload_pbar", pct)
              }
              line <- paste(
                step,
              ...
              .update_status("upload_status_line", line)
          ...
          .update_status("upload_status_line", paste("Monitor error:", conditionMessage(e)))
          updateRunnerFooter(TRUE)
          observerHandle$destroy()
  ...
  observeEvent(input$show_shortcuts, {
    ...
    # Generate keyboard shortcuts HTML dynamically from config
    ...
    showModalSafe(modalDialog(
      ...
      tags$div(
        ...
        # Dynamic keyboard shortcuts from config/keyboard_shortcuts.json
        keyboard_shortcuts_html,
        
        # Static user guide content from external HTML file
        includeHTML(file.path(epc_path("www_dir"), "user_guide.html"))
      )
    ))
  })
  ...
  # Local VERSION file (stamped by GitHub Action on release) takes precedence.
  # Falls back to GitHub API. Compares both to show "Update available" banner.
  
  # Cache for version info (fetched once per session)
  .about_cache <- new.env(parent = emptyenv())
  .about_cache$local   <- NULL
  .about_cache$remote  <- NULL
  .about_cache$checked <- FALSE
  ...
  observeEvent(input$show_about, {
    ...
    if (!.about_cache$checked) {
      .about_cache$remote <- tryCatch({
        resp <- httr::GET(
          "https://api.github.com/repos/RSloot2000/epic2castor_public/releases/latest",
          httr::timeout(5)
        )
      ...
      .about_cache$checked <- TRUE
    }
    
    local_ver  <- .about_cache$local
    remote_ver <- .about_cache$remote
    
    # Determine display version: local VERSION file wins, else GitHub latest
    display_ver <- if (!is.na(local_ver)) local_ver
                   else if (!is.na(remote_ver)) remote_ver
                   else "unknown"
    
    # Build version HTML
    version_html <- tags$p(
      style = "text-align: center; color: #999; font-size: 12px; margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee;",
      icon("tag"), " Version: ", tags$strong(display_ver)
    )
    
    # "Update available" banner (only when local VERSION exists and remote is newer)
    update_html <- NULL
    if (!is.na(local_ver) && !is.na(remote_ver) && local_ver != remote_ver) {
      # Strip leading "v" for numeric comparison
      local_clean  <- sub("^v", "", local_ver)
      remote_clean <- sub("^v", "", remote_ver)
      if (utils::compareVersion(remote_clean, local_clean) > 0) {
        update_html <- tags$div(
          style = "text-align: center; background: #fff3cd; border: 1px solid #ffc107; border-radius: 6px; padding: 10px; margin-top: 10px;",
          icon("arrow-circle-up", style = "color: #e67e22;"),
          " Update available: ",
          tags$strong(remote_ver),
          tags$br(),
          tags$a(
            href = "https://github.com/RSloot2000/epic2castor_public/releases/latest",
            target = "_blank", rel = "noopener",
            style = "color: #0366d6; font-size: 12px;",
    ...
    showModalSafe(modalDialog(
      ...
      tagList(
        includeHTML(file.path(epc_path("www_dir"), "about.html")),
        version_html,
        update_html
      )
    ))
  ...
  observeEvent(input$close_modal_lab_data, {
    removeModal()
  })
  ...
  render_table <- function(data, file, mode = c("auto", "full", "proxy"), resetPaging = FALSE) {
    ...
    for (i in seq_along(option_data_selected)) {
      ...
      if (any(colnames(display_data) == display_column)) {
        display_data[[display_column]] <- sapply(seq_len(nrow(display_data)), function(valueNumber) {
          ...
          # --- Determine cell status via centralized function ---
          full_row <- as.list(data_with_delete[valueNumber, , drop = FALSE])
          cell_info <- determine_cell_status(
            value          = value,
            options_list   = optionsList,
            column         = column,
            row_idx        = valueNumber,
            file           = file,
            duplicate_info = duplicateElements(),
            manual_vals    = manualValues$vals,
            full_row       = full_row
          )
          css_class <- cell_info$css_class
          tooltip   <- cell_info$tooltip
          ...
          tooltip_attr <- if (nzchar(tooltip)) paste0('data-tooltip="', htmltools::htmlEscape(tooltip), '" ') else ""
      ...
      output$table <- renderDT({
        datatable(
          ...
          options = c(dt_options, list(  # Merge adaptive options with callbacks
            ...
            initComplete = JS(
              # Select2 init delegated to initializeSelect2Elements via drawCallback
              # (drawCallback fires right after initComplete — no need to double-init)
    ...
    # Notify JavaScript to reload Select2 dropdowns
    session$sendCustomMessage("reloadSelect2", list())
    
    # Only restore scroll position after full renders (page reload / table switch)
    # Proxy updates (dropdown changes) should NOT trigger scroll restore —
    # it can permanently shift the table under the fixed header
  ...
  # When deleting from elements table, also removes related checkbox/radiobutton data
  observeEvent(input$delete_rows, {
    ...
    if (!is.null(new_data) && nrow(new_data) > 0) {
      ...
      # Capture Element values of rows being deleted BEFORE removing them
      # Needed for cascading delete of related checkbox/radiobutton data
      deleted_elements <- NULL
      if (input$file == "elements" && "Element" %in% names(new_data)) {
        deleted_elements <- new_data[selected_ids, Element]
        deleted_elements <- deleted_elements[!is.na(deleted_elements) & deleted_elements != ""]
      }
      ...
      # Cascading delete: remove related checkboxes and radiobuttons
      if (input$file == "elements" && length(deleted_elements) > 0) {
        tab_name  <- get_active_tab_name()
        tab_order <- get_active_tab_order()
        
        # Remove related checkboxes
        checkbox_data <- mappingData[["waarde_checkboxes"]]
        if (!is.null(checkbox_data) && nrow(checkbox_data) > 0) {
          if ("tab_name_meta" %in% names(checkbox_data)) {
            keep <- !(checkbox_data$Element %in% deleted_elements &
                      checkbox_data$tab_name_meta == tab_name &
                      checkbox_data$tab_order_meta == tab_order)
          } else {
            keep <- !(checkbox_data$Element %in% deleted_elements)
          }
          mappingData[["waarde_checkboxes"]] <<- checkbox_data[keep, ]
        }
        
        # Remove related radiobuttons
        radio_data <- mappingData[["waarde_radiobuttons"]]
        if (!is.null(radio_data) && nrow(radio_data) > 0) {
          if ("tab_name_meta" %in% names(radio_data)) {
            keep <- !(radio_data$Element %in% deleted_elements &
                      radio_data$tab_name_meta == tab_name &
                      radio_data$tab_order_meta == tab_order)
          } else {
            keep <- !(radio_data$Element %in% deleted_elements)
          }
          mappingData[["waarde_radiobuttons"]] <<- radio_data[keep, ]
        }
        
        cat(sprintf("[Delete] Removed %d element(s) and cascaded to checkboxes/radiobuttons (tab: %s)\n",
                    length(deleted_elements), tab_name))
      }
  ...
  observeEvent(input$dropdown_change, {
    ...
    if(info$value == "__ADD__") {
      ...
      # Send dynamic color + tooltip update for the changed cell
      full_row <- as.list(new_data[row, , drop = FALSE])
      cell_info <- determine_cell_status(
        value          = info$value,
        options_list   = character(0),   # options not available here; render_table handles full re-color
        column         = colName,
        row_idx        = row,
        file           = input$file,
        duplicate_info = duplicateElements(),
      ...
      session$sendCustomMessage("updateCellColor", list(
        row = row,
        col = colName,
        cssClass = cell_info$css_class,
        tooltip = cell_info$tooltip
      ))
    }
  })
  ...
  observeEvent(input$modal_save, {
    ...
    # Persist to SQLite so manual values survive app restart
    save_manual_value(input$file, key, input$new_value)
    ...
    # Use determine_cell_status for the correct color (manual → blue)
    full_row <- as.list(new_data[currentEdit$row, , drop = FALSE])
    cell_info <- determine_cell_status(
      value          = input$new_value,
      options_list   = character(0),   # not in any list → will fall through to manual check
      column         = currentEdit$col,
      row_idx        = currentEdit$row,
      file           = input$file,
      duplicate_info = duplicateElements(),
      manual_vals    = manualValues$vals,
      full_row       = full_row
  ...
  observeEvent(input$select_epic_file, {
    ...
    wizard_rv$export_complete <- FALSE
    wizard_rv$current_step <- 1
    # SharePoint / password state reset
    wizard_rv$file_source <- "local"
    wizard_rv$sharepoint_path <- NULL
    wizard_rv$excel_password <- NULL
    wizard_rv$password_required <- FALSE
    wizard_rv$temp_export_path <- NULL
    wizard_rv$source_path <- NULL
    wizard_rv$temp_source_path <- NULL
    # Detect SharePoint/OneDrive roots once on open
    wizard_rv$sharepoint_roots <- tryCatch(detect_sharepoint_roots(), error = function(e) NULL)

    # Reset the fileInput so no previous file is shown
    shinyjs::reset("wizard_file_upload")

    # Build dynamic import type choices from config
    # get_enabled_import_types() returns named vector: id -> display_name
    import_type_choices <- get_enabled_import_types()
      ...
      tagList(
        tags$head(
          tags$style(HTML("
            ...
            .wizard-progress li.wiz-active {
              color: #007bff;
              font-weight: bold;
            }
            .wizard-progress li.wiz-active::before {
              background: #007bff;
              color: #fff;
            }
            .wizard-progress li.wiz-complete {
              color: #28a745;
              cursor: pointer;
            }
            .wizard-progress li.wiz-complete::before {
              content: '\\2713';
              background: #28a745;
              color: #fff;
            }
            .wizard-progress li.wiz-complete::after {
              background: #28a745;
            }
            .wizard-progress li.wiz-active::after {
        ...
        div(id = "wizard_step_1",
          div(class = "wizard-step",
            ...
            fluidRow(
              ...
              column(6,
                radioButtons("wizard_file_source", "File Source:",
                  choices = c("Local upload" = "local", "SharePoint / OneDrive" = "sharepoint"),
                  selected = "local", inline = TRUE)
              )
            ),
            # Local upload panel
            conditionalPanel(
              condition = "input.wizard_file_source == 'local'",
            ...
            conditionalPanel(
              condition = "input.wizard_file_source == 'sharepoint'",
              div(style = "padding: 8px 0;",
                uiOutput("wizard_sharepoint_root_ui"),
                fluidRow(
                  column(9,
                    textInput("wizard_sharepoint_path", "File path:",
                      placeholder = "Full path or select root above, then browse...",
                      width = "100%")
                  ),
                  column(3, style = "margin-top: 25px;",
                    shinyFiles::shinyFilesButton("wizard_sp_browse", "Browse...",
                      title = "Select a file",
                      multiple = FALSE,
                      buttonType = "default",
                      class = "btn-sm",
                ...
                uiOutput("wizard_sp_password_ui")
              )
            ),
        ...
        # Step 2: Detection (Dynamic)
        div(id = "wizard_step_2", style = "display: none;",
          uiOutput("wizard_detection_ui")
        ),

        # Step 3: Mapping (Dynamic)
        div(id = "wizard_step_3", style = "display: none;",
          uiOutput("wizard_mapping_ui")
        ),
        
        # Step 4: Preview & Export
        div(id = "wizard_step_4", style = "display: none;",
          uiOutput("wizard_preview_ui")
        )
      ),
      footer = tagList(
        modalButton("Close"),
        actionButton("wizard_prev_step", "Previous", class = "btn btn-default", icon = icon("arrow-left"),
                     style = "display: none;"),
        actionButton("wizard_next_step", "Next", class = "btn btn-primary", icon = icon("arrow-right"))
      ),
      size = "l",
      easyClose = FALSE,
      class = "wizard-modal"
  ...
  wizard_rv <- reactiveValues(
    ...
    export_result = NULL,
    current_step = 1,
    # SharePoint / password support
    file_source = "local",          # "local" or "sharepoint"
    sharepoint_roots = NULL,        # named character vector from detect_sharepoint_roots()
    sharepoint_path = NULL,         # full resolved path to SharePoint file
    excel_password = NULL,          # password for protected Excel (cleared on close)
    password_required = FALSE,      # TRUE when detect detects password protection
    temp_export_path = NULL,        # tempdir export path for SharePoint mode
    source_path = NULL,             # original SharePoint path (for reload)
    temp_source_path = NULL,        # temp copy of raw source file
    export_complete = FALSE          # TRUE after successful export (for progress bar)
  )

  # ===== SHAREPOINT SESSION STATE (persists across wizard opens) =====
  sp_session <- reactiveValues(
    source_path = NULL,           # original SharePoint/OneDrive file path
    password = NULL,              # Excel password for automatic reload
    temp_source_path = NULL,      # path to temp copy of raw source file
    last_import_type = NULL,      # e.g. "epic_baseline"
    last_mapping_df = NULL,       # last used mapping dataframe
    last_detection_result = NULL, # for sheet selection on reload
    last_selected_sheets = NULL   # sheets used in last import
  )

  # ===== WIZARD: PROGRESS BAR =====
  output$wizard_progress_bar <- renderUI({
    step <- wizard_rv$current_step
    step_labels <- c("Upload", "Verification", "Mapping", "Export")
    items <- lapply(seq_along(step_labels), function(i) {
      exported <- isTRUE(wizard_rv$export_complete)
      cls <- if (i == 4 && exported) "wiz-complete" else if (i < step) "wiz-complete" else if (i == step) "wiz-active" else ""
      onclick <- if (i < step) {
        sprintf("Shiny.setInputValue('wizard_goto_step', %d, {priority: 'event'});", i)
      } else {
        ""
      }
      tags$li(class = cls, onclick = onclick, step_labels[i])
    })
    tags$ul(class = "wizard-progress", items)
  })

  # ===== WIZARD: SHAREPOINT UI =====
  # Render SharePoint root selector (auto-detected OneDrive/SP folders)
  output$wizard_sharepoint_root_ui <- renderUI({
    roots <- wizard_rv$sharepoint_roots
    if (is.null(roots) || length(roots) == 0) {
      # No auto-detected roots — manual path entry only
      tags$p(
        icon("info-circle"),
        " No SharePoint/OneDrive folder detected. Enter the full file path manually.",
        style = "color: #856404; font-size: 0.9em;"
      )
    } else {
      root_choices <- c("Select a root folder..." = "", roots)
      selectInput("wizard_sp_root", "SharePoint/OneDrive root:",
        choices = root_choices, width = "100%")
    }
  })

  # Render password input (only shown when the file is actually password-protected)
  output$wizard_sp_password_ui <- renderUI({
    # Only show when password_required has been detected
    if (!isTRUE(wizard_rv$password_required)) return(NULL)
    
    div(style = "margin-top: 8px;",
      div(style = "padding: 8px; background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 4px; margin-bottom: 8px;",
        tags$p(style = "color: #856404; margin: 0; font-size: 0.9em;",
          icon("lock"),
          " This Excel file is password-protected."
        )
      ),
      passwordInput("wizard_sp_password", "Excel password:",
        placeholder = "Enter password...", width = "100%")
    )
  })

  # Update textInput path when user selects a SharePoint root
  observeEvent(input$wizard_sp_root, {
    root_val <- input$wizard_sp_root
    if (!is.null(root_val) && nzchar(root_val)) {
      updateTextInput(session, "wizard_sharepoint_path", value = root_val)
    }
  })

  # Track file_source radio changes
  observeEvent(input$wizard_file_source, {
    wizard_rv$file_source <- input$wizard_file_source
    # Reset file state when switching source
    wizard_rv$file_path <- NULL
    wizard_rv$file_name <- NULL
    wizard_rv$detection_result <- NULL
    wizard_rv$available_sheets <- NULL
  ...
  # SharePoint browse button — register shinyFiles handler (top-level, called once)
  # Uses the selected root from the dropdown, or falls back to all roots / home
  shinyFiles::shinyFileChoose(input, "wizard_sp_browse",
    roots = function() {
      all_roots <- wizard_rv$sharepoint_roots
      selected  <- input$wizard_sp_root
      # If user selected a specific root in the dropdown, use only that one
      if (!is.null(selected) && nzchar(selected) && !is.null(all_roots)) {
        match_idx <- which(all_roots == selected)
        if (length(match_idx) > 0) {
          return(all_roots[match_idx[1]])
        }
      }
      if (is.null(all_roots) || length(all_roots) == 0) {
        home <- normalizePath(Sys.getenv("USERPROFILE"), winslash = "/", mustWork = FALSE)
        return(c(Home = home))
      }
      all_roots
    },
  ...
  # Capture shinyFiles selection
  observeEvent(input$wizard_sp_browse, {
    all_roots <- wizard_rv$sharepoint_roots
    selected  <- input$wizard_sp_root
    # Mirror the same root logic as the shinyFileChoose registration
    if (!is.null(selected) && nzchar(selected) && !is.null(all_roots)) {
      match_idx <- which(all_roots == selected)
      if (length(match_idx) > 0) {
        roots <- all_roots[match_idx[1]]
      } else {
        roots <- all_roots
      }
    } else if (is.null(all_roots) || length(all_roots) == 0) {
      home <- normalizePath(Sys.getenv("USERPROFILE"), winslash = "/", mustWork = FALSE)
      roots <- c(Home = home)
    } else {
      roots <- all_roots
    }
    file_selected <- tryCatch(
      shinyFiles::parseFilePaths(roots = roots, input$wizard_sp_browse),
      error = function(e) data.frame()
    )
    if (nrow(file_selected) > 0) {
      full_path <- as.character(file_selected$datapath[1])
      updateTextInput(session, "wizard_sharepoint_path", value = full_path)
    }
  }, ignoreInit = TRUE)

  # When SharePoint path is confirmed (user presses Enter or moves away), validate file
  observeEvent(input$wizard_sharepoint_path, {
    if (wizard_rv$file_source != "sharepoint") return()
    sp_path <- input$wizard_sharepoint_path
    if (is.null(sp_path) || !nzchar(trimws(sp_path))) {
      wizard_rv$file_path <- NULL
      wizard_rv$file_name <- NULL
      return()
    }
    sp_path <- trimws(sp_path)
    # Skip validation if path is a directory (e.g. root folder selection)
    if (dir.exists(sp_path)) return()
    ...
    if (!ext %in% c("csv", "xlsx", "xls")) {
      showNotification("Only .csv and .xlsx files are supported", type = "warning")
      return()
    }
    wizard_rv$file_path <- sp_path
    wizard_rv$file_name <- basename(sp_path)
    wizard_rv$sharepoint_path <- sp_path
    # Reset password state for new file selection
    wizard_rv$password_required <- FALSE
    wizard_rv$excel_password <- NULL
    ...
    # Handle Excel sheets — detect password protection in background
    if (ext %in% c("xlsx", "xls")) {
      sheets <- tryCatch({
        readxl::excel_sheets(sp_path)
      }, error = function(e) {
        # Any failure to read sheets likely means password-protected
        # (matches detect_file_type() behaviour)
        wizard_rv$password_required <- TRUE
        NULL
      })
  ...
  # Show/hide step divs when current_step changes
  observeEvent(wizard_rv$current_step, {
    for (s in 1:4) {
      if (s == wizard_rv$current_step) {
        shinyjs::show(id = paste0("wizard_step_", s))
      } else {
        shinyjs::hide(id = paste0("wizard_step_", s))
      }
    }
    # Update navigation button visibility
    if (wizard_rv$current_step > 1) {
      shinyjs::show(id = "wizard_prev_step")
    } else {
      shinyjs::hide(id = "wizard_prev_step")
    }
    # Hide "Next" on the last step
    if (wizard_rv$current_step >= 4) {
      shinyjs::hide(id = "wizard_next_step")
    } else {
      shinyjs::show(id = "wizard_next_step")
    }
  })

  # Next button
  observeEvent(input$wizard_next_step, {
    step <- wizard_rv$current_step
    ...
    if (step == 1) {
      ...
      # Use the user-selected import type — only verify against that type
      selected_type <- input$wizard_import_type
      if (is.null(selected_type) || !selected_type %in% names(IMPORT_CONFIGS)) {
        showNotification("Please select a valid import type", type = "warning")
        return()
      }
      single_config <- IMPORT_CONFIGS[selected_type]

      # Resolve password from SharePoint mode or NULL for local
      active_password <- NULL
      if (wizard_rv$file_source == "sharepoint") {
        sp_pw <- input$wizard_sp_password
        if (!is.null(sp_pw) && nzchar(sp_pw)) active_password <- sp_pw
        wizard_rv$excel_password <- active_password
      }
      
      shinyjs::disable("wizard_next_step")
      showNotification("Verifying file structure...", type = "message", duration = NULL, id = "wizard_analyzing")
      tryCatch({
        # SharePoint mode: check password requirement, then copy to session temp
        analysis_path <- wizard_rv$file_path
        analysis_password <- active_password

        if (wizard_rv$file_source == "sharepoint") {
          # Pre-flight: check if password-protected before copying
          ft <- detect_file_type(wizard_rv$file_path, password = NULL)
          if (isTRUE(ft$password_protected) && (is.null(active_password) || !nzchar(active_password))) {
            removeNotification("wizard_analyzing")
            wizard_rv$password_required <- TRUE
            showNotification("This file is password-protected. Please enter the password.", type = "warning", duration = 6)
            shinyjs::enable("wizard_next_step")
            return()
          }
          # Copy (and decrypt if password) to session temp
          wizard_rv$source_path <- wizard_rv$file_path
          temp_copy <- copy_to_session_temp(wizard_rv$file_path, password = active_password)
          wizard_rv$temp_source_path <- temp_copy
          wizard_rv$file_path <- temp_copy
          # Temp copy is unprotected — no password needed for subsequent reads
          analysis_path <- temp_copy
          analysis_password <- NULL
        }
        ...
        det <- detect_file_structure(
          ...
          import_configs = single_config,
          password = analysis_password
        )
        # Handle password_required early return from detect_file_structure
        if (isTRUE(det$password_required)) {
          removeNotification("wizard_analyzing")
          wizard_rv$password_required <- TRUE
          showNotification("This file is password-protected. Please enter the password in Step 1.", type = "warning", duration = 6)
          shinyjs::enable("wizard_next_step")
          return()
        }
        mt <- selected_type
        det$structure$matched_type <- mt
        det$structure$matched_name <- if (!is.null(IMPORT_CONFIGS[[mt]]$name)) IMPORT_CONFIGS[[mt]]$name else mt
        wizard_rv$detection_result <- det
        wizard_rv$password_required <- FALSE
        removeNotification("wizard_analyzing")
        wizard_rv$current_step <- 2
      }, error = function(e) {
        removeNotification("wizard_analyzing")
      ...
      if (is.null(wizard_rv$detection_result)) {
        showNotification("Please run detection first", type = "warning")
        return()
      }
      wizard_rv$current_step <- 3
    } else if (step == 3) {
      # Validate mapping before going to step 4
      ...
      shinyjs::disable("wizard_next_step")
      tryCatch({
        # Collect latest mapping values from inputs
        for (i in seq_len(nrow(wizard_rv$mapping_df))) {
          val <- input[[paste0("wizard_map_col_", i)]]
          if (!is.null(val)) {
            wizard_rv$mapping_df$file_column[i] <- val
            wizard_rv$mapping_df$is_mapped[i] <- (val != "")
            wizard_rv$mapping_df$match_type[i] <- ifelse(val == "", "", "manual")
          }
        }
  ...
  # Previous button
  observeEvent(input$wizard_prev_step, {
    if (wizard_rv$current_step > 1) {
      wizard_rv$current_step <- wizard_rv$current_step - 1
    }
  })

  # Clickable completed steps in progress bar
  observeEvent(input$wizard_goto_step, {
    target <- as.integer(input$wizard_goto_step)
    if (!is.na(target) && target >= 1 && target < wizard_rv$current_step) {
      wizard_rv$current_step <- target
    }
  })
  
  # ===== WIZARD: FILE INFO =====
  output$wizard_file_info_ui <- renderUI({
    # For local mode: react to fileInput; for SharePoint: react to wizard_rv$file_path
    if (wizard_rv$file_source == "sharepoint") {
      # SharePoint mode — file_path already set by sharepoint_path observer
      req(wizard_rv$file_path)
      sp_path <- wizard_rv$file_path
      file_size <- tryCatch(file.info(sp_path)$size, error = function(e) NA)
      pw_note <- if (isTRUE(wizard_rv$password_required) && is.null(wizard_rv$excel_password)) {
        tags$p(icon("lock"), " This file appears to be password-protected. Enter the password above.",
          style = "color: #856404; margin: 3px 0; font-size: 0.9em;")
      } else NULL
      
      return(tags$div(
        style = "margin-top: 10px; padding: 10px; background: #d4edda; border: 1px solid #c3e6cb; border-radius: 4px;",
        p(icon("check-circle"), " File selected: ", tags$strong(wizard_rv$file_name),
          style = "color: #155724; margin: 0;"),
        if (!is.na(file_size)) p(sprintf("Size: %.1f KB", file_size / 1024),
          style = "color: #155724; margin: 3px 0; font-size: 0.9em;"),
        tags$p(icon("cloud"), " Source: SharePoint/OneDrive",
          style = "color: #155724; margin: 3px 0; font-size: 0.9em;"),
        pw_note
      ))
    }
  ...
  output$wizard_detection_ui <- renderUI({
    ...
    selected_display_name <- if (!is.null(result$structure$matched_name)) {
    ...
    # Build column match summary from structure indicators
    match_info <- result$structure$indicators
    matched_cols <- sum(unlist(match_info), na.rm = TRUE)
    total_cols <- length(match_info)
    
    div(class = "wizard-step step-complete",
      h3("Step 2: File Verification"),
      fluidRow(
        column(6,
          tags$strong("Import Type: "),
          tags$br(),
          tags$span(selected_display_name, style = "font-size: 1.1em; font-weight: bold;")
        ),
        column(6,
          tags$strong("Match Score: "),
          tags$br(),
      ...
      hr(),
      # Show skip-rows info if header was not on row 1
      if (!is.null(result$file_info$skip_rows) && result$file_info$skip_rows > 0) {
        tags$div(
          style = "padding: 8px; background-color: #d1ecf1; border: 1px solid #bee5eb; border-radius: 4px; margin-bottom: 10px;",
          tags$p(style = "color: #0c5460; margin: 0; font-size: 0.9em;",
            icon("info-circle"),
            sprintf(" Header detected on row %d (skipping %d metadata row%s)",
                    result$file_info$skip_rows + 1,
                    result$file_info$skip_rows,
                    if (result$file_info$skip_rows > 1) "s" else ""))
        )
      },
      fluidRow(
        column(4, p(tags$strong("Columns in file: "), length(result$structure$col_names))),
        column(4, p(tags$strong("Rows preview: "), result$structure$n_rows)),
        column(4, p(tags$strong("Required matched: "), 
          sprintf("%d / %d", matched_cols, total_cols)))
      ),
      # Show which required columns matched/failed
      if (total_cols > 0) {
        tags$div(
          style = "margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 4px; font-size: 0.9em;",
          tags$strong("Required columns:"),
          tags$ul(
            lapply(names(match_info), function(col_name) {
              clean_name <- sub("^has_", "", col_name)
              if (isTRUE(match_info[[col_name]])) {
                tags$li(style = "color: #28a745;", icon("check"), clean_name)
              } else {
                tags$li(style = "color: #dc3545;", icon("times"), clean_name, " — not found")
              }
            })
          )
        )
      ...
      tags$details(
        ...
        tags$summary(style = "cursor: pointer; font-weight: bold;", "Show file columns"),
        tags$div(
          style = "max-height: 150px; overflow-y: auto; background: #f8f9fa; padding: 10px; border-radius: 4px; font-size: 0.85em; margin-top: 5px;",
          tags$ul(
            lapply(head(result$structure$col_names, 30), function(col) {
              tags$li(code(col))
            }),
            if (length(result$structure$col_names) > 30) {
              tags$li(sprintf("... and %d more", length(result$structure$col_names) - 30))
            }
          )
        )
      ),
        ...
        fluidRow(
          column(8,
            selectInput(
              ...
              label = tags$span(icon("exchange-alt"), " Change Import Type:"),
              choices = import_type_choices,
  ...
  observeEvent(input$wizard_apply_override, {
    req(wizard_rv$detection_result, input$wizard_override_import_type, wizard_rv$file_path)

    new_type <- input$wizard_override_import_type
    old_type <- wizard_rv$detection_result$structure$matched_type

    if (identical(new_type, old_type)) {
      showNotification("Import type is already set to this value", type = "warning", duration = 4)
      return()
    }

    # Re-verify the file against the new type
    single_config <- IMPORT_CONFIGS[new_type]
    tryCatch({
      det <- detect_file_structure(
        wizard_rv$file_path,
        sheet = if (!is.null(wizard_rv$selected_sheets) && length(wizard_rv$selected_sheets) == 1) wizard_rv$selected_sheets[1] else wizard_rv$selected_sheet,
      ...
      det$structure$matched_type <- new_type
      det$structure$matched_name <- if (!is.null(IMPORT_CONFIGS[[new_type]]$name)) {
      ...
      showNotification(
        sprintf("Re-verified as: %s (%d%% match)", det$structure$matched_name, det$confidence$score),
        type = "message",
        duration = 5
      )
    }, error = function(e) {
  ...
  .wiz_col_obs <- new.env(parent = emptyenv()); .wiz_col_obs$handles <- list()
  observe({
    req(wizard_rv$mapping_df, wizard_rv$detection_result)

    # Destroy previous column mapping observers to prevent accumulation
    for (obs in .wiz_col_obs$handles) try(obs$destroy(), silent = TRUE)
    .wiz_col_obs$handles <- list()

    lapply(seq_len(nrow(wizard_rv$mapping_df)), function(i) {
      input_id <- paste0("wizard_map_col_", i)

      h <- observeEvent(input[[input_id]], {
      ...
      .wiz_col_obs$handles[[length(.wiz_col_obs$handles) + 1L]] <- h
    })
  })
      ...
      fluidRow(
        ...
        column(4,
          div(style = "margin-top: 25px;",
            tags$a(class = "template-toggle-btn", 
                   `data-toggle` = "collapse", href = "#wizard_template_panel",
                   icon("cog"), " Manage Templates")
          )
        )
      ),
      # Collapsible template management panel
      div(id = "wizard_template_panel", class = "collapse",
        style = "background-color: #e9ecef; padding: 10px; margin-bottom: 15px; border-radius: 5px;",
        fluidRow(
          column(6,
          ...
          column(6,
            tags$label(icon("list"), " Saved Templates",
  ...
  observeEvent(input$wizard_export, {
    ...
    # In SharePoint mode, export to temp unless user chose "save locally"
    custom_export_path <- NULL
    save_locally <- isTRUE(input$wizard_save_locally) && wizard_rv$file_source == "sharepoint"
    if (wizard_rv$file_source == "sharepoint" && !save_locally) {
      temp_base <- file.path(tempdir(), "epc_sharepoint")
      import_type <- wizard_rv$transformation_result$log$import_type
      # Mirror the standard export directory structure inside temp
      sub_dir <- switch(import_type,
        epic_baseline = "input_data/epic_export",
        biobank_data  = "input_data/biobank_data",
        follow_up     = "input_data/follow_up",
        lab_data      = "input_data/lab_data",
        mdns_mapping  = "input_data/biobank_data",
        mapping_elements = "mapping",
        file.path("input_data", import_type)
      )
      export_dir <- file.path(temp_base, sub_dir)
      if (!dir.exists(export_dir)) dir.create(export_dir, recursive = TRUE)
      # Use standard filename
      filename <- switch(import_type,
        ...
        paste0(import_type, "_export.csv")
      )
      custom_export_path <- file.path(export_dir, filename)
      wizard_rv$temp_export_path <- temp_base
    }
    ...
    res <- tryCatch({
      export_transformation_result(
        ...
        format = "csv",
        custom_path = custom_export_path
      )
    }, error = function(e) {
    ...
    if (isTRUE(res$success)) {
      ...
      # For SharePoint mode (temp export), set EPC_TEMP_INPUT_DIR so option_lists2.R and ETL subprocesses read from temp
      # When saved locally, no env var override needed — standard paths apply
      if (wizard_rv$file_source == "sharepoint" && !save_locally && !is.null(wizard_rv$temp_export_path)) {
        Sys.setenv(EPC_TEMP_INPUT_DIR = wizard_rv$temp_export_path)
      }
      # If saved locally, clear any previous temp override
      if (save_locally && nzchar(Sys.getenv("EPC_TEMP_INPUT_DIR", ""))) {
        Sys.unsetenv("EPC_TEMP_INPUT_DIR")
      }
      ...
      # Persist SharePoint session state for reload functionality
      if (wizard_rv$file_source == "sharepoint" && !is.null(wizard_rv$source_path)) {
        sp_session$source_path <- wizard_rv$source_path
        sp_session$password <- wizard_rv$excel_password
        sp_session$temp_source_path <- wizard_rv$temp_source_path
        sp_session$last_import_type <- wizard_rv$transformation_result$log$import_type
        sp_session$last_mapping_df <- wizard_rv$mapping_df
        sp_session$last_detection_result <- wizard_rv$detection_result
        sp_session$last_selected_sheets <- wizard_rv$selected_sheets
        cat(sprintf("[Import Wizard] SharePoint session state saved for: %s\n", basename(wizard_rv$source_path)))
      }
      wizard_rv$export_complete <- TRUE
    } else {
  ...
  output$wizard_export_result <- renderUI({
    ...
    if (isTRUE(res$success)) {
      sp_note <- if (wizard_rv$file_source == "sharepoint") {
        if (isTRUE(input$wizard_save_locally)) {
          tags$p(style = "color: #155724; font-size: 0.9em; margin-top: 5px;",
            icon("hdd"),
            " Data imported from SharePoint/OneDrive and saved locally to input_data/."
          )
        } else {
          tags$p(style = "color: #155724; font-size: 0.9em; margin-top: 5px;",
            icon("cloud"),
            " Data imported from SharePoint/OneDrive (temp copy). ",
            "Use File > 'Reload SharePoint data' to refresh from source."
          )
        }
      } else NULL
  ...
  # ===== SHAREPOINT RELOAD =====
  # Conditional menu item — only shown when a SharePoint import was completed
  output$sp_reload_menu_item <- renderUI({
    if (is.null(sp_session$source_path)) return(NULL)
    actionLink("sp_reload_data", 
      paste0("Reload SharePoint data (", sp_session$last_import_type, ")"),
      class = "menu-link", icon = icon("refresh"))
  })

  # Reload observer — re-copies source, re-transforms with saved mapping, re-exports
  observeEvent(input$sp_reload_data, {
    req(sp_session$source_path, sp_session$last_mapping_df, sp_session$last_import_type)

    # Prevent reload during ETL run
    if (isTRUE(runnerState$running)) {
      showNotification("Cannot reload while a process is running.", type = "warning")
      return()
    }

    showNotification("Reloading data from SharePoint...", type = "message", duration = NULL, id = "sp_reload_notify")
    tryCatch({
      # 1. Re-copy from source (re-decrypt if needed)
      temp_copy <- reload_from_source(sp_session$source_path, password = sp_session$password)
      sp_session$temp_source_path <- temp_copy

      # 2. Re-detect file structure (no password — temp is unprotected)
      det <- detect_file_structure(
        temp_copy,
        sheet = if (!is.null(sp_session$last_selected_sheets) && length(sp_session$last_selected_sheets) == 1) sp_session$last_selected_sheets[1] else NULL,
        sheets = if (!is.null(sp_session$last_selected_sheets) && length(sp_session$last_selected_sheets) > 1) sp_session$last_selected_sheets else NULL,
        import_configs = IMPORT_CONFIGS[sp_session$last_import_type],
        password = NULL
      )
      det$structure$matched_type <- sp_session$last_import_type
      det$structure$matched_name <- if (!is.null(IMPORT_CONFIGS[[sp_session$last_import_type]]$name)) IMPORT_CONFIGS[[sp_session$last_import_type]]$name else sp_session$last_import_type
      sp_session$last_detection_result <- det

      # 3. Re-transform with saved mapping
      transform_result <- transform_import_data(
        temp_copy,
        det,
        sp_session$last_mapping_df,
        sheet = if (!is.null(sp_session$last_selected_sheets) && length(sp_session$last_selected_sheets) == 1) sp_session$last_selected_sheets[1] else NULL,
        sheets = if (!is.null(sp_session$last_selected_sheets) && length(sp_session$last_selected_sheets) > 1) sp_session$last_selected_sheets else NULL,
        password = NULL
      )

      if (is.null(transform_result$transformed_data)) {
        removeNotification("sp_reload_notify")
        showNotification("Reload failed: transformation returned no data.", type = "error", duration = 6)
        return()
      }

      # 4. Re-export to temp directory
      temp_base <- file.path(tempdir(), "epc_sharepoint")
      import_type <- sp_session$last_import_type
      sub_dir <- switch(import_type,
        epic_baseline = "input_data/epic_export",
        biobank_data  = "input_data/biobank_data",
        follow_up     = "input_data/follow_up",
      ...
      res <- export_transformation_result(
        transform_result,
        format = "csv",
        custom_path = custom_export_path
      )
      ...
      if (!isTRUE(res$success)) {
        removeNotification("sp_reload_notify")
        showNotification(paste("Reload export failed:", res$message), type = "error", duration = 6)
        return()
      }

      # 5. Reload option lists
      Sys.setenv(EPC_TEMP_INPUT_DIR = temp_base)
      tryCatch({
        options(epic2castor.force_option_reload = TRUE)
        local_env <- new.env(parent = globalenv())
        sys.source(file.path(epc_path("scripts_dir"), "option_lists2.R"), envir = local_env)
        ...
        cat(sprintf("[SP Reload] Warning: Failed to reload option lists: %s\n", conditionMessage(e)))
      })

      removeNotification("sp_reload_notify")
      showNotification(
        sprintf("Data refreshed from SharePoint (%s, %d rows)", 
          basename(sp_session$source_path), 
          nrow(transform_result$transformed_data)),
        type = "message", duration = 5
      )
      cat(sprintf("[SP Reload] Successfully reloaded: %s\n", basename(sp_session$source_path)))
    }, error = function(e) {
      removeNotification("sp_reload_notify")
      showNotification(sprintf("Reload failed: %s", e$message), type = "error", duration = 8)
      cat(sprintf("[SP Reload] Error: %s\n", e$message))
    })
  })
  ...
  output$wizard_preview_ui <- renderUI({
    ...
    div(class = "wizard-step step-active",
      ...
      div(
        ...
        # Save locally option for SharePoint mode
        if (wizard_rv$file_source == "sharepoint") {
          div(
            style = "margin-top: 10px; padding: 10px; background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 5px;",
            checkboxInput("wizard_save_locally", 
              label = tags$span(
                icon("hdd"), 
                " Save file locally (copy to input_data/)"
              ),
              value = FALSE),
            tags$p(
              style = "color: #856404; font-size: 0.85em; margin: 5px 0 0 25px; font-weight: bold;",
              icon("exclamation-triangle"),
              " WARNING: Do NOT save files containing personal data (persoonsgegevens) locally. ",
              "Only use this option for anonymized or non-sensitive data."
            )
          )
        },
