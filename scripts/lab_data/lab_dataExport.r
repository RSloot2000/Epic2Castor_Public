## Castor export voor Lab data (gebaseerd op biobankExport.r)
## Gebruik: Rscript scripts/lab_data/lab_dataExport.r "<site_id> - <site_name>" (site optioneel)
...
if (exists("epic2castor_status_init")) {
	try(epic2castor_status_init(total = NA_integer_, title = "Lab Data Upload", step = "init", detail = "Starting"), silent = TRUE)
}
...
# Bouw een httr2 POST request met retry + throttle
build_post_req <- function(url, token, payload_json) {
	httr2::request(url) |>
		...
		httr2::req_error(is_error = function(resp) FALSE)  # handle errors ourselves
}

# Bouw een httr2 GET request met retry
...
# Voer een lijst httr2 requests parallel uit (max 6 concurrent)
...
csv_file <- file.path(epc_path("lab_output_data_dir"), "lab_data.csv")
if (!file.exists(csv_file)) stop("File not found: ", csv_file)
data <- read.csv2(csv_file, stringsAsFactors = FALSE)

# Lab data CSV heeft kolommen al als Castor variabelen: Participant.Id, collection_date, lab_*
# Er is geen extra Elements mapping nodig (in tegenstelling tot biobank)
if (!("Participant.Id" %in% names(data))) {
	stop("De kolom 'Participant.Id' ontbreekt in ", csv_file)
}

# Bepaal lab_* velden (alle kolommen behalve Participant.Id en collection_date)
meta_cols <- c("Participant.Id", "collection_date")
lab_field_cols <- setdiff(names(data), meta_cols)
cat("Found", length(lab_field_cols), "lab field columns and", nrow(data), "rows in", csv_file, "\n")

# Schatting aantal datapoints
num_fields_per_record <- length(lab_field_cols) + 1  # +1 voor collection_date
...
if (exists("epic2castor_status_update")) {
	try(epic2castor_status_update(step = "prepare", total = dp_total_estimate,
																detail = paste("Loaded", nrow(data), "rows from lab_data.csv"),
																force = TRUE), silent = TRUE)
}

# Bouw records: elke rij wordt een record met Participant.Id + alle velden
...
json_file <- file.path(epc_path("castor_export_dir"), "lab_data.json")
...
unique_participants <- unique(as.character(data$Participant.Id))
...
# Forceer expliciete repeating data naam
rd_target_name <- "MyCoS Lab"
...
lab_json <- fromJSON(json_file, simplifyVector = FALSE)
records <- lab_json$records
records_by_participant <- split(records, sapply(records, function(rec) rec$fields$Participant.Id))

# Waarde normalisatie voor upload (Castor: d-m-Y voor date)
format_castor_value <- function(field_var, value) {
	...
	is_dateonly <- !is.null(field_var) && grepl("(^|_)date($|_)|datum|_date$|^date_", field_var, ignore.case = TRUE)
	...
	if (is_dateonly || grepl("^\\d{1,2}-\\d{1,2}-\\d{4}$", s) || grepl("^\\d{4}-\\d{1,2}-\\d{1,2}$", s)) {
		if (grepl("^\\d{4}-\\d{1,2}-\\d{1,2}$", s)) {
			d <- strsplit(s, "-")[[1]]
			return(paste0(pad2(d[3]), "-", pad2(d[2]), "-", sprintf("%04d", as.integer(d[1]))))
		}
		if (grepl("^\\d{1,2}-\\d{1,2}-\\d{4}$", s)) {
			d <- strsplit(s, "-")[[1]]
			return(paste0(pad2(d[1]), "-", pad2(d[2]), "-", sprintf("%04d", as.integer(d[3]))))
		}
	}
...
cached_instances   <- list()   # participant_id -> filtered instances payload
new_instance_ids   <- list()   # participant_id -> character vector van NIEUWE instance IDs
...
for (participant_id in names(records_by_participant)) {
	...
	# Filter alleen instances die bij Lab Data horen
	lab_instances_list <- Filter(function(inst) {
		!is.null(inst$repeating_data_name) && inst$repeating_data_name == repeating_data_name
	}, all_instances_list)

	aantal_bestaand <- length(lab_instances_list)
	cat("Total instances:", length(all_instances_list), "| Lab instances:", aantal_bestaand, "\n")
	...
	if (aantal_bestaand < aantal_nodig) {
		...
		if (aantal_bestaand > 0) {
			for (inst in lab_instances_list) {
				inst_name <- inst$name
		...
		# Verwerk resultaten: check op fouten en verzamel nieuwe instance IDs
		...
		for (k in seq_along(create_resps)) {
			...
			if (inherits(resp, "httr2_response") && httr2::resp_status(resp) == 201L) {
				...
				if (!is.null(new_id)) {
					...
					# Voeg toe aan lab_instances_list zodat cache compleet is
					lab_instances_list <- c(lab_instances_list, list(list(
	...
	cached_instances[[participant_id]] <- list(`_embedded` = list(repeatingDataInstance = lab_instances_list))
}
...
for (participant_id in names(records_by_participant)) {
	...
	cat("Lab instances:", length(id_by_index), "| New:", length(new_ids), "for participant", participant_id, "\n")
	...
	existing_maps <- list()  # index -> existing_map
	...
	for (i in seq_along(rec_list)) {
		...
		for (field_var in names(record_data)) {
			new_field_id <- unname(mapping_list[field_var])
			...
			raw_value <- record_data[[field_var]]
			field_value <- format_castor_value(field_var, raw_value)
			...
			fields_metadata[[new_field_id]] <- field_var
		}

		if (length(fields_to_upload) > 0) {
			# Filter unchanged fields lokaal (ipv in batch_upload_helper)
			skipped_count <- 0
		...
		for (k in seq_along(post_resps)) {
			...
			if (inherits(resp, "httr2_response") && httr2::resp_status(resp) %in% c(200L, 201L, 207L)) {
				...
				if (n_fail > 0) {
					...
					# Fallback: retry failed fields individueel
				...
				# Volledige batch failure - fallback per veld
