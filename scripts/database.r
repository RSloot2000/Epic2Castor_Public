...
# - Waarde = tolower(substr(eerste woord Form Name, 1, 4)) _ Field Variable Name
generate_pv_elements <- function(inFile = file.path(castor_meta_dir, "study_variablelist.csv"),
  ...
  # Prefix samenstellen uit Form Name (eerste woord, max 4 tekens)
  prefix <- tolower(substr(sub(" .*", "", svl[["Form Name"]]), 1, 4))
