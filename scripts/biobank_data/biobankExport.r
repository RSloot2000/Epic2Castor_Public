...
# Output CSV has Castor column names already (mapped by biobank_data.r).
# Identify participant ID column and biobank field columns.
pid_col_name <- if ("Participant.Id" %in% names(data)) {
  "Participant.Id"
} else if ("Participant Id" %in% names(data)) {
  "Participant Id"
} else if ("Identificatie" %in% names(data)) {
  "Identificatie"
} else {
  stop("Geen participant ID kolom gevonden in ", csv_file)
}

meta_cols <- c(pid_col_name)
biobank_field_cols <- setdiff(names(data), meta_cols)

# Schatting aantal datapoints (ruwe indicatie)
num_fields_per_record <- length(biobank_field_cols)
...
# Bouw records: kolommen zijn al Castor-veldnamen, plus Participant.Id
records <- lapply(seq_len(nrow(data)), function(i) {
	row <- data[i, , drop = FALSE]
	fields <- list()
	fields[["Participant.Id"]] <- as.character(row[[pid_col_name]])
	for (col in biobank_field_cols) {
		val <- row[[col]]
		if (!is.na(val) && nzchar(as.character(val))) {
			fields[[col]] <- val
		}
	}
	list(record_id = as.character(i), fields = fields)
})
...
unique_participants <- unique(as.character(data[[pid_col_name]]))
...
for (participant_id in names(records_by_participant)) {
	...
	# Filter alleen instances die bij deze repeating data structure (biobank) horen
	biobank_instances_list <- Filter(function(inst) {
		!is.null(inst$repeating_data_name) && inst$repeating_data_name == repeating_data_name
	}, all_instances_list)
	
	aantal_bestaand <- length(biobank_instances_list)
	cat("Total instances:", length(all_instances_list), "| Biobank instances:", aantal_bestaand, "\n")
	cat("Existing biobank instances for participant", participant_id, ":", aantal_bestaand, "\n")
	...
	if (aantal_bestaand < aantal_nodig) {
		...
		if (aantal_bestaand > 0) {
			for (inst in biobank_instances_list) {
				inst_name <- inst$name
		...
		cat("Highest existing biobank instance index:", max_existing_index, "\n")
		...
		for (k in seq_along(create_resps)) {
			...
			if (inherits(resp, "httr2_response") && httr2::resp_status(resp) == 201L) {
				...
				if (!is.null(new_id)) {
					...
					biobank_instances_list <- c(biobank_instances_list, list(list(
	...
	cached_instances[[participant_id]] <- list(`_embedded` = list(repeatingDataInstance = biobank_instances_list))
}
...
for (participant_id in names(records_by_participant)) {
	...
	cat("Biobank instances:", length(id_by_index), "| New:", length(new_ids), "for participant", participant_id, "\n")
