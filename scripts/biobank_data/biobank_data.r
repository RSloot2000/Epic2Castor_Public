...
tryCatch({
	...
	in_dir  <- epc_path("biobank_input_data_dir")
	out_dir <- epc_path("biobank_output_data_dir")
	...
	# ---- Default biobank column mapping: Epic name -> Castor variable ----------
	# These defaults are overridden by entries in elements.csv (tab_name_meta = "Biobank")
	biobank_default_mapping <- c(
		"iLES.sampleId"    = "biob_bb_iles_sampleid",
		"Tube.sampleId"    = "biob_bb_tube_sampleid",
		"Monstersoort"     = "biob_bb_sample_type",
		"Status"           = "biob_bb_status",
		"Afname.datumtijd" = "biob_bb_collection_date_time",
		"Afgerond"         = "biob_bb_finished"
	)
	biobank_col_mapping <- epc_load_column_mapping("Biobank", biobank_default_mapping)
	message(sprintf("Biobank mapping: %d entries (incl. elements.csv overrides)", length(biobank_col_mapping)))
	...
	# Excel origin for serial date conversion
	...
	normalize_date <- function(x) {
		...
		conv <- function(v) {
			...
			# Excel serial number (pure integer or numeric with no fractional part)
			num <- suppressWarnings(as.numeric(v))
			if (!is.na(num) && num > 30000 && num < 100000) {
				d <- as.Date(num, origin = excel_origin)
	...
	normalize_datetime <- function(x) {
		...
		conv <- function(v) {
			if (is.na(v)) return(NA_character_)
			# Excel serial number with optional fractional day (e.g. 44662.399305)
			num <- suppressWarnings(as.numeric(v))
			if (!is.na(num) && num > 30000 && num < 100000) {
				d <- as.Date(floor(num), origin = excel_origin)
				frac <- num - floor(num)
				secs <- round(frac * 86400)
				hh <- secs %/% 3600
				mm <- (secs %% 3600) %/% 60
				mm <- round_quarter(mm)
				return(paste0(format(d, "%d-%m-%Y"), ";", pad2(hh), ":", pad2(mm)))
			}
			vv <- gsub("[./]", "-", v)
	...
	# ---- Rename columns to Castor variable names using mapping -----------------
	# Identificatie -> "Participant Id" (apart), rest via biobank_col_mapping
	current_names <- names(bb_out)
	new_names <- current_names
	# Rename Identificatie column
	new_names[new_names == ident_col] <- "Participant Id"
	# Rename mapped biobank columns
	for (i in seq_along(new_names)) {
		if (new_names[i] %in% names(biobank_col_mapping)) {
			new_names[i] <- biobank_col_mapping[new_names[i]]
		}
	}
	names(bb_out) <- new_names

	# Update adt_col/fin_col references to new names for quoting logic
	if (!is.null(adt_col) && adt_col %in% names(biobank_col_mapping)) adt_col <- biobank_col_mapping[adt_col]
	if (!is.null(fin_col) && fin_col %in% names(biobank_col_mapping)) fin_col <- biobank_col_mapping[fin_col]
