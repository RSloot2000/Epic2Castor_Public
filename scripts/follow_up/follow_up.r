...
# Folders (centralized)
inputPath    <- paste0(epc_path("follow_up_input_data_dir"), "/")
outputFolder <- paste0(epc_path("follow_up_output_data_dir"), "/")
...
try({
  ...
  epic2castor_status_init(total = total_tables, title = "Follow-up", step = "init", detail = "Starting…")
}, silent = TRUE)
...
# Filter alleen velden met 'foll_' voorvoegsel voor follow-up
...
followup_element_tabs <- unique(elements[, c("Element", "tab_name_meta")])
waarde_checkboxes <- merge(waarde_checkboxes, followup_element_tabs, by = c("Element", "tab_name_meta"))
waarde_radiobuttons <- merge(waarde_radiobuttons, followup_element_tabs, by = c("Element", "tab_name_meta"))
...
tryCatch({
  for (table in seq_len(nrow(scheme))){
    ...
    if (file.exists(bestandXLSX)) {
      ...
      inputData <- readr::read_csv2(
        bestandCSV, 
        col_types = cols(datum = col_character(), .default = col_guess()), 
    ...
    if ("datum" %in% colnames(inputData)){
      if (typeof(inputData$datum) == "character"){
        # Try dd-mm-YYYY first; fall back to Excel serial number conversion
        parsed <- as.Date(inputData$datum, format = "%d-%m-%Y")
        still_na <- is.na(parsed) & !is.na(inputData$datum) & nzchar(inputData$datum)
        if (any(still_na)) {
          nums <- suppressWarnings(as.numeric(inputData$datum[still_na]))
          parsed[still_na] <- as.Date(nums, origin = "1899-12-30")
        }
        inputData$datum <- parsed
      } else {
        # Numeric: assume Excel serial numbers (origin 1899-12-30)
        inputData$datum <- as.Date(as.numeric(inputData$datum), origin = "1899-12-30")
      }
    } else {
    ...
    # --- Verwijder baseline aggregatie voor follow-up (geen aggregatie nodig) ---
    # Follow-up data wordt niet geaggregeerd, elke visit blijft een aparte rij
    
    # Filter: alleen MyCoS Participant IDs behouden (bijv. MyCoS0001, MyCoS0346)
    if ("Participant Id" %in% names(outputData)) {
      before_n <- nrow(outputData)
      keep_mycos <- grepl("^MyCoS\\d+$", outputData$`Participant Id`)
      outputData <- outputData[keep_mycos, , drop = FALSE]
      removed_n <- before_n - nrow(outputData)
      if (removed_n > 0) cat(sprintf("Filtered out %d non-MyCoS Participant IDs (kept %d)\n", removed_n, nrow(outputData)))
    }
    ...
    # Voor follow-up data gebruiken we altijd "follow_up.csv" als output naam
    output_filename <- "follow_up"
    fwrite(as.data.table(outputData), 
      paste0(outputFolder, output_filename, ".csv"), 
      sep = ";", 
      na = "", 
      bom = TRUE)
    cat("Successfully saved: ", outputFolder, output_filename, ".csv\n", sep = "")
    # Update progress to this completed table (force write for UI)
    try(epic2castor_status_update(step = "write", detail = paste0("Saved ", output_filename, ".csv"), current = table, force = TRUE), silent = TRUE)
  ...
  if (startsWith(err_msg, "ERROR:FILE_NOT_FOUND:")) {
    display_msg <- paste("Follow-up script mislukt: een benodigd inputbestand kon niet worden gevonden.")
  } else {
    display_msg <- paste("Follow-up script mislukt:", err_msg)
  }
...
# Disconnect from the databases - wrap in try() om crashes te voorkomen
try({
  if (exists("conn_mapping") && !is.null(conn_mapping)) {
    tryCatch(dbDisconnect(conn_mapping), error = function(e) invisible(NULL))
  }
}, silent = TRUE)

try({
  if (exists("conn_meta") && !is.null(conn_meta)) {
    tryCatch(dbDisconnect(conn_meta), error = function(e) invisible(NULL))
  }
}, silent = TRUE)
...
try({
  ...
  try(epic2castor_status_done(paste0("Follow-up finished — Log saved to ", logfile)), silent = TRUE)
}, silent = TRUE)

# Expliciete cleanup om Windows crash bij exit te voorkomen
try({
  # Verwijder ALLE objecten behalve reserved names
  rm(list = setdiff(ls(all.names = TRUE), c()))
  gc(verbose = FALSE, full = TRUE)
}, silent = TRUE)

# Extra gc() call om zeker te zijn
try(gc(verbose = FALSE, full = TRUE), silent = TRUE)

# Flush alle output streams voor we exit
flush(stdout())
flush(stderr())

# Kleine pauze voor I/O completion
Sys.sleep(0.1)
